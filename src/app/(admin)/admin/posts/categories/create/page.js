...
export default function CreateCategory() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch(`/api/posts/categories/create`, {
        method: "POST",
        body: JSON.stringify({
  ...
  return (
    <>
      <h2 className="text-3xl font-bold mb-10">Create Category</h2>
      ...
      {status === "error" && (
        <AlertBox
          status="error"
          message="There was an error creating the category. Please try again later."
        ></AlertBox>
      )}
